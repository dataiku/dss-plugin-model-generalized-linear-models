    ...
    <q-card style="width: 400px">
      <q-card-section>
        <div class="row items-center q-gutter-x-sm no-wrap">
          ...
          <div class="dku-grand-title">{{ title }}</div>
        </div>
      </q-card-section>
      ...
      <q-card-actions align="right">
        <bs-button class="bs-btn-flat dku-text" unelevated flat @click="$emit('cancel')">
          {{ t('cancel') }}</bs-button
        >
        <bs-button class="bs-btn-flat-danger dku-text" unelevated flat @click="$emit('confirm')">{{
          t('delete')
        }}</bs-button>
      </q-card-actions>
    </q-card>
...
.card-text {
  ...
  color: var(--greyscale-grey-lighten-2, #444);
}
</style>
